...
*/
	...
	function sshos_theme_customizer( $wp_customize ) {
        ...
        /* logo height */
        $wp_customize->add_setting('sshos_logo_height',array('sanitize_callback' => 'sanitize_sshos_logo_height',));
        function sanitize_sshos_logo_height($input) {return wp_kses_post(force_balance_tags($input));}
        $wp_customize->add_control('sshos_logo_height',array(
            'type' => 'text',
        ...
        //
        // ADD "Get more features!" SECTION TO "Simple Sticky Header on Scroll Plugin" PANEL 
        //
        $wp_customize->add_section('sshos_gopro_section',array('title' => esc_html__( 'Get more features!', 'sshos' ), 'panel' => 'sshos_panel','priority' => 10));

        require_once 'custom_controls.php';
        /* custom notice custom contol */
        $wp_customize->add_setting('custom_info', array(
            'default'           => '',
            'sanitize_callback' => 'pageloader_sshos_sanitization',
        ));
        $wp_customize->add_control(new Info_Custom_control($wp_customize, 'custom_info', array(
            'settings'		=> 'custom_info',
            'section'  		=> 'sshos_gopro_section',
        )));
...
<?php
	...
	//
    add_filter( 'plugin_action_links_' . plugin_basename(__FILE__), 'add_action_links_sshos' );
    function add_action_links_sshos ( $links ) {
        $mylinks = array(
        '<a href="' . admin_url( 'customize.php?autofocus[panel]=sshos_panel' ) . '">Settings</a>',
        '<a href="https://codecanyon.net/item/sticky-header-on-scroll-for-wordpress/20259567" target="_blank" style="color:red;">Go Pro!</a>',
        );
	...
	?>
		...
		**************************************************************/
        ...
        /* logo size */
        .sshos-logo-wrapper .sshos-logo-inner img { height:<?php echo get_theme_mod('sshos_logo_height'); ?>px; }
        /* logo colors */
